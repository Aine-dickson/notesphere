    ...
    <div>
        ...
        <div v-else class="flex flex-col -space-y-4 rtl:space-x-reverse">
            <div v-for="(participant, index) in participants.slice(0, 5)" :key="index" class="rounded-full border-2 border-white dark:border-gray-800 overflow-hidden h-14 w-14">
                ...
                <span @click="cameraStore.preview_participants" v-else class="flex cursor-pointer items-center justify-center w-fbg-gray-700 ull h-full text-xs font-medium text-white bg-gray-700 " href="#">+{{ participants.length-4 }}</span>
            </div>
        </div>
    </div>
</template>
